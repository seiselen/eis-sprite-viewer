...
// private final static String APP_BRND = "©2024 Eiselen Technologies ";
    ...
    UIAppBar appBar = new UIAppBar(app, WidgetType.NA)
    .bindAppLogoΘ(AppMain.APPLOGO, null);
    uim.bindUiObject(appBar);
  ...
  //=[CUR CLIP/FRAME LABELS GROUP]==================================================  
    ...
    UIContainer.create(
      ...
      UILabel.create(app, bbox(vec(96,yOff+=32), vec(168,32)),
        "Frame:", AppFont.TEXT, LabelType.TP, null
      ).setStyleProp("strk_transp", Integer.class, tspt)
      .setStyleProp("fill_transp", Integer.class, tspt)
      .setStyleProp("txt_anchor",  PosOri.class,  PosOri.RGT)
      .setStyleProp("txt_offset",  PVector.class, v.copy()),
      ...
      UILabel.create(app, bbox(vec(96,yOff+=32), vec(168,32)),
        null, AppFont.TEXT, LabelType.TP, new DispCurFrmProg(appUtil)
      ).setStyleProp("strk_transp", Integer.class, tspt)
      .setStyleProp("fill_transp", Integer.class, tspt)
      .setStyleProp("txt_anchor",  PosOri.class,  PosOri.LFT)
      .setStyleProp("txt_offset",  PVector.class, v.copy())
  ...
  //=[FPS BUTTON GROUP]=========================================================
    ...
    UIContainer.create(
      ...
      UIClick.create(app, new BBox(xOff,0, eDim,eDim),
        glyphChar("gFPSDec"), AppFont.GLYPH, new FpsIncAction(AppMain.fpsManager)
      ).setTitle("Decrease Playback Speed (i.e. Increase Virtual FPS Factor)"),

      UIClick.create(app, new BBox(xOff+=eDim+xPad, 0, eDim, eDim), 
        glyphChar("gFPSDef"), AppFont.GLYPH, new FpsDefAction(AppMain.fpsManager)
      ).setTitle("Reset Playback Speed (i.e. set Virtual FPS Factor to [1])"),

      UIClick.create(app, new BBox(xOff+=eDim+xPad, 0, eDim, eDim), 
        glyphChar("gFPSInc"), AppFont.GLYPH, new FpsDecAction(AppMain.fpsManager)
      ).setTitle("Increase Playback Speed (i.e. Decrease Virtual FPS Factor)"),

      UILabel.create(app, new BBox(xOff+=eDim+xPad, 0, eDim, eDim),
        null, AppFont.TEXT, LabelType.OP, new FpsGetUpdate(AppMain.fpsManager)
      ).setTitle("Current Virtual FPS Factor")
  ...
  //=[LOAD TARGET BUTTON]==================================================================
    ...
    UIClick.create(
      uim, new BBox(AppMain.CANVAS_WIDE-360, 8, 160, 40),
      "Load Target", AppFont.TEXT, new LoadTargetAction()
    )
    .setTitle("Load Anindefs File")
...
class FpsIncAction implements IActionCallback {
  private VFPSUtil fm;  
  public FpsIncAction(VFPSUtil ifm){fm=ifm;}  
  public void action(){fm.incTarDF(1);}
}

class FpsDecAction implements IActionCallback {
  private VFPSUtil fm;  
  public FpsDecAction(VFPSUtil ifm){fm=ifm;}  
  public void action(){fm.decTarDF(1);}
}

class FpsDefAction implements IActionCallback {
  private VFPSUtil fm;  
  public FpsDefAction(VFPSUtil VFPSUtil){fm=VFPSUtil;} 
  public void action(){fm.setTarDF(VFPSUtil.DEF_DF);}
}

class FpsGetUpdate implements IUpdateCallback {
  private VFPSUtil fm;    
  public FpsGetUpdate(VFPSUtil VFPSUtil){fm=VFPSUtil;} 
  public String getTxt() {return fm.tarFPSToString();}
}
...
class DispCurClip implements IUpdateCallback {
  ...
  public String getTxt(){return appUtils.hasCurSpriteGroup() ? appUtils.curSpriteGroup.getCurClipName() : "N/A";}
}

class DispCurSprt implements IUpdateCallback {
  private AppUtils appUtils;
  public DispCurSprt(AppUtils iAppUtils){appUtils=iAppUtils;}
  public String getTxt(){return appUtils.hasCurSpriteGroup() ? appUtils.curSpriteGroup.getCurSpriteName() : "N/A";}
}

class DispCurSpOff implements IUpdateCallback {
  private AppUtils appUtils;
  public DispCurSpOff(AppUtils iAppUtils){appUtils=iAppUtils;}
  public String getTxt(){return appUtils.hasCurSpriteGroup() ? appUtils.curSpriteGroup.getCurSpriteOff() : "N/A";}
}

class DispCurFrmProg implements IUpdateCallback {
  private AppUtils appUtils;
  public DispCurFrmProg(AppUtils iAppUtils){appUtils=iAppUtils;}
  public String getTxt(){return appUtils.hasCurSpriteGroup() ? appUtils.curSpriteGroup.clipProgressToString() : "N/A";}
}